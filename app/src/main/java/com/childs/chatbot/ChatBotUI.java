...
public class ChatBotUI extends AppCompatActivity {
        ...
        Thread thread = new Thread(new Runnable(){
            public void run() {
                try {
                    ...
                    assistantservice.setUsernameAndPassword("apikey", getResources().getString(R.string.assistant_apikey));
                    ...
                    assistantservice.setEndPoint(getResources().getString(R.string.assistant_url));
                    InputData input = new InputData.Builder(inputmessage).build();
                    //WORKSPACES are now SKILLS
                    MessageOptions options = new MessageOptions.Builder().workspaceId(getResources().getString(R.string.assistant_id)).input(input).context(context).build();
    ...
    private void recordMessage() {
        ...
        if(listening != true) {
            ...
            listening = true;
            Toast.makeText(ChatBotUI.this,"Listening....Click to Stop", Toast.LENGTH_LONG).show();

        } else {
            try {
                microphoneHelper.closeInputStream();
                listening = false;
                Toast.makeText(ChatBotUI.this,"Stopped Listening....Click to Start", Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void showError(final Exception e) {
        runOnUiThread(new Runnable() {
            @Override public void run() {
                Toast.makeText(ChatBotUI.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        });
    }
